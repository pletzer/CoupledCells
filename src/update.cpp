...
void communication_async_send_recv(grid_parms grid, double** sendbuf, double** recvbuf, SMC_type&  smc, EC_type&  ec)
{
...
// Specific to KNBGR.
void communication_update_sendbuf(grid_parms grid, double** sendbuf, SMC_type&  smc, EC_type&  ec)
{
	int k, buf_offset;
	...
	for (int i = (int) sendbuf[UP][0]; i <= (int) sendbuf[UP][1]; i++) {
		int j = 1;
		sendbuf[UP][buf_offset + k + 0] = smc.var(i, j, smc_Ca);
		sendbuf[UP][buf_offset + k + 1] = smc.var(i, j, smc_Vm);
		sendbuf[UP][buf_offset + k + 2] = smc.var(i, j, smc_IP3);
	...
	for (int i = (int) sendbuf[UP][2]; i <= (int) sendbuf[UP][3]; i++) {
		int j = 1;
		sendbuf[UP][buf_offset + k + 0] = ec.var(i, j, ec_Ca);
		sendbuf[UP][buf_offset + k + 1] = ec.var(i, j, ec_Vm);
		sendbuf[UP][buf_offset + k + 2] = ec.var(i, j, ec_IP3);
	...
	for (int i = (int) sendbuf[DOWN][0]; i <= (int) sendbuf[DOWN][1]; i++) {
		...
		sendbuf[DOWN][buf_offset + k + 0] = smc.var(i, j, smc_Ca);
		sendbuf[DOWN][buf_offset + k + 1] = smc.var(i, j, smc_Vm);
		sendbuf[DOWN][buf_offset + k + 2] = smc.var(i, j, smc_IP3);
		k += grid.num_coupling_species_smc;
	}
	...
	for (int i = (int) sendbuf[DOWN][2]; i <= (int) sendbuf[DOWN][3]; i++) {
		...
		sendbuf[DOWN][buf_offset + k + 0] = ec.var(i, j, ec_Ca);
		sendbuf[DOWN][buf_offset + k + 1] = ec.var(i, j, ec_Vm);
		sendbuf[DOWN][buf_offset + k + 2] = ec.var(i, j, ec_IP3);
		k += grid.num_coupling_species_ec;
	}
	...
	for (int j = (int) sendbuf[LEFT][0]; j <= (int) sendbuf[LEFT][1]; j++) {
		int i = 1;
		sendbuf[LEFT][buf_offset + k + 0] = smc.var(i, j, smc_Ca);
		sendbuf[LEFT][buf_offset + k + 1] = smc.var(i, j, smc_Vm);
		sendbuf[LEFT][buf_offset + k + 2] = smc.var(i, j, smc_IP3);
		k += grid.num_coupling_species_smc;
	}
	...
	for (int j = (int) sendbuf[LEFT][2]; j <= (int) sendbuf[LEFT][3]; j++) {
		int i = 1;
		sendbuf[LEFT][buf_offset + k + 0] = ec.var(i, j, ec_Ca);
		sendbuf[LEFT][buf_offset + k + 1] = ec.var(i, j, ec_Vm);
		sendbuf[LEFT][buf_offset + k + 2] = ec.var(i, j, ec_IP3);
		k += grid.num_coupling_species_ec;
	}
	...
	for (int j = (int) sendbuf[RIGHT][0]; j <= (int) sendbuf[RIGHT][1]; j++) {
		...
		sendbuf[RIGHT][buf_offset + k + 0] = smc.var(i, j, smc_Ca);
		sendbuf[RIGHT][buf_offset + k + 1] = smc.var(i, j, smc_Vm);
		sendbuf[RIGHT][buf_offset + k + 2] = smc.var(i, j, smc_IP3);
		k += grid.num_coupling_species_smc;
	}
	...
	for (int j = (int) sendbuf[RIGHT][2]; j <= (int) sendbuf[RIGHT][3]; j++) {
		...
		sendbuf[RIGHT][buf_offset + k + 0] = ec.var(i, j, ec_Ca);
		sendbuf[RIGHT][buf_offset + k + 1] = ec.var(i, j, ec_Vm);
		sendbuf[RIGHT][buf_offset + k + 2] = ec.var(i, j, ec_IP3);
		k += grid.num_coupling_species_ec;
	}
}
...
/*
 ...
 Specific to KNBGR.
*/
void communication_update_recvbuf(grid_parms grid, double** recvbuf, SMC_type&  smc, EC_type&  ec)
{
	int k, buf_offset;
	...
	for (int i = (int) recvbuf[UP][0]; i <= (int) recvbuf[UP][1]; i++) {
		...
		smc.var(i, j, smc_Vm) = recvbuf[UP][buf_offset + k + 1];
		smc.var(i, j, smc_IP3) = recvbuf[UP][buf_offset + k + 2];
		k += grid.num_coupling_species_smc;
	}
	...
	for (int i = (int) recvbuf[UP][2]; i <= (int) recvbuf[UP][3]; i++) {
		int j = 0;
		ec.var(i, j, ec_Ca) = recvbuf[UP][buf_offset + k + 0];
		ec.var(i, j, ec_Vm) = recvbuf[UP][buf_offset + k + 1];
		ec.var(i, j, ec_IP3) = recvbuf[UP][buf_offset + k + 2];
		k += grid.num_coupling_species_ec;
	}
	...
	if (grid.flip_array[DOWN] == 0)
	{
		for (int i = (int) recvbuf[DOWN][0]; i <= (int) recvbuf[DOWN][1]; i++) {
			...
			smc.var(i, j, smc_Ca) = recvbuf[DOWN][buf_offset + k + 0];
			smc.var(i, j, smc_Vm) = recvbuf[DOWN][buf_offset + k + 1];
			smc.var(i, j, smc_IP3) = recvbuf[DOWN][buf_offset + k + 2];
			k += grid.num_coupling_species_smc;
		}
	}
	...
	if (grid.flip_array[DOWN] == 0) {
		for (int i = (int) recvbuf[DOWN][2]; i <= (int) recvbuf[DOWN][3];
			...
			ec.var(i, j, ec_Ca) = recvbuf[DOWN][buf_offset + k + 0];
			ec.var(i, j, ec_Vm) = recvbuf[DOWN][buf_offset + k + 1];
			ec.var(i, j, ec_IP3) = recvbuf[DOWN][buf_offset + k + 2];
			k += grid.num_coupling_species_ec;
		}
	...
	for (int j = (int) recvbuf[LEFT][0]; j <= (int) recvbuf[LEFT][1]; j++) {
		int i = 0;
		smc.var(i, j, smc_Ca) = recvbuf[LEFT][buf_offset + k + 0];
		smc.var(i, j, smc_Vm) = recvbuf[LEFT][buf_offset + k + 1];
		smc.var(i, j, smc_IP3) = recvbuf[LEFT][buf_offset + k + 2];
		k += grid.num_coupling_species_smc;
	}
	...
	for (int j = (int) recvbuf[LEFT][2]; j <= (int) recvbuf[LEFT][3]; j++) {
		int i = 0;
		ec.var(i, j, ec_Ca) = recvbuf[LEFT][buf_offset + k + 0];
		ec.var(i, j, ec_Vm) = recvbuf[LEFT][buf_offset + k + 1];
		ec.var(i, j, ec_IP3) = recvbuf[LEFT][buf_offset + k + 2];
		k += grid.num_coupling_species_ec;
	}
	...
	for (int j = (int) recvbuf[RIGHT][0]; j <= (int) recvbuf[RIGHT][1]; j++) {
		...
		smc.var(i, j, smc_Ca) = recvbuf[RIGHT][buf_offset + k + 0];
		smc.var(i, j, smc_Vm) = recvbuf[RIGHT][buf_offset + k + 1];
		smc.var(i, j, smc_IP3) = recvbuf[RIGHT][buf_offset + k + 2];
		k += grid.num_coupling_species_smc;
	}
	...
	for (int j = (int) recvbuf[RIGHT][2]; j <= (int) recvbuf[RIGHT][3]; j++) {
		...
		ec.var(i, j, ec_Ca) = recvbuf[RIGHT][buf_offset + k + 0];
		ec.var(i, j, ec_Vm) = recvbuf[RIGHT][buf_offset + k + 1];
		ec.var(i, j, ec_IP3) = recvbuf[RIGHT][buf_offset + k + 2];
		k += grid.num_coupling_species_ec;
	}
}
...
void read_init_ATP(grid_parms *grid, EC_type& ECs)
{
	...
	for(int m = 1; m <= grid->num_ec_circumferentially; m++)
	{
		for(int n = 1; n <= grid->num_ec_axially; n++)
		{
			...
			ECs.JPLC(m, n) = recv_jplc[(n - 1) * grid->num_ec_circumferentially + m - 1];
		}
	}
