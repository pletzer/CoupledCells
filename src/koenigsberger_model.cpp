/*
 * koenigsberger_model.cpp
 *
 *  Created on: 12/06/2013
 *      Author: mohsinshaikh
 */

#include <math.h>
#include <cstdlib>
#include "computelib.h"
#include "koenigsberger_model.h"

const double
	/* Constants for homogenically coupled SMCs. */
      Fi = 0.23,  Kri = 1.00,  GCai = 0.00129,  vCa1 = 100.00,
	  vCa2 = -24.00,  RCai = 8.50,  GNaCai = 0.00316,  cNaCai = 0.5,
	  vNaCai = -30.00,  Bi = 2.025,  cbi = 1.0,  CICRi = 55.00,
	  sci = 2.0,  cci = 0.9,  Di = 0.24,  vdi = -100.00,
	  Rdi = 250.00,  Li = 0.025, gama = 1970.00,
	  FNaK = 0.0432,  GCli = 0.00134,  vCl = -25.0,
	  GKi = 0.0046,  vKi = -94.00,  lambda = 45.00,
	  cwi = 0.0,  beta = 0.13,  vCa3 = -27.0,
	  RKi = 12.0,  ki = 0.1,
	/* Constants for homogenically coupled ECs. */
	  Fj = 0.23,  Krj = 1.00,  Bj = 0.5,
	  cbj = 1.0,  CICRj = 5.0,  scj = 2.0,
	  ccj = 0.9,  Dj = 0.24,  Lj = 0.025,
	  kj = 0.1,  Gcatj = 0.66 * 1e-3,  ECa = 50.00,
	  m3cat = -0.18,
	  m4cat = 0.37,  J0j = 0.029,  Cmj = 25.8,
	  Gtot = 6927,   vKj = -80.0,  a1j = 53.3,
	  a2j = 53.3,     bj = -80.8,  c1j = -0.4,
	  m3b = 1.32e-3, m4b = 0.30,   m3s = -0.28,
	  m4s = 0.389,   GRj = 955, vrestj = -31.10,
	/*Intracellular calcium buffering*/
	  // k6 = 100.00,  k7 = 300.00,  BT = 120.00;

	// Lemon at al. Constants
	kATP = 2, alpha_j = 2.781e-5, KCa = 0.4, R_PIP2_r = 10,
	PIP2_tot = 4e7, K_G_prot_act = 0.017, delta = 1.234e-3,
	G_prot_tot = 1e5, K_G_prot_deact = 0.15, N_a = 6.02252e23,
	V_ec = 1.17e3, unitcon_a = 1e21, PIP2_C = 5e7, kDeg = 1.25,
	cons_PIP2 = 3.989e7;


/// Initial values found by running a sufficiently long simulation and recording state values
/// after they have reached a steady state.
void initialize_koenigsberger_smc(const grid_parms& grid, double* __restrict__ y, SMC_type&  __restrict__ smc)
{
	int k = 0, offset;
	srand(grid.universal_rank);

	for (int i = 1; i <= grid.num_smc_circumferentially; i++) {
		for (int j = 1; j <= grid.num_smc_axially; j++) {
			k = (i - 1) * grid.neq_smc_axially;

			if (grid.random)
			{
				y[k + ((j - 1) * grid.neq_smc) + smc_Ca] = (double)rand() / (double)(RAND_MAX / 0.1) + 0.15;
				y[k + ((j - 1) * grid.neq_smc) + smc_SR] = (double)rand() / (double)(RAND_MAX / 0.2) + 1.3;
				y[k + ((j - 1) * grid.neq_smc) + smc_Vm] = (double)rand() / (double)(RAND_MAX / 5.0) - 68.0;
				y[k + ((j - 1) * grid.neq_smc) + smc_w] = (double)rand() / (double)(RAND_MAX / 0.005) + 0.01;
				y[k + ((j - 1) * grid.neq_smc) + smc_IP3] = (double)rand() / (double)(RAND_MAX / 0.2) + 0.6;
			}
			else
			{
				y[k + ((j - 1) * grid.neq_smc) + smc_Ca] = 0.15;
				y[k + ((j - 1) * grid.neq_smc) + smc_SR] = 1.3;
				y[k + ((j - 1) * grid.neq_smc) + smc_Vm] = - 68.0;
				y[k + ((j - 1) * grid.neq_smc) + smc_w] = 0.01;
				y[k + ((j - 1) * grid.neq_smc) + smc_IP3] = 0.6;
			}

		}
	}
#pragma ivdep
#pragma omp for simd collapse(2)
	int nc = grid.num_smc_circumferentially + grid.num_ghost_cells;
    int na = grid.num_smc_axially + grid.num_ghost_cells;
    for (int ij = 0; ij < nc * na; ij++) {
        int i = ij / na;
        int j = ij % na;
        smc.var(i, j, smc_Ca) = 0.0;
        smc.var(i, j, smc_SR) = 0.0;
        smc.var(i, j, smc_Vm) = 0.0;
        smc.var(i, j, smc_w) = 0.0;
        smc.var(i, j, smc_IP3) = 0.0;

        // TODO: remove initialising fluxes for lemon model even when not used....
        for (int k = 0; k < grid.num_fluxes_smc; k++) {
            smc.flux(i, j, k) = 0.1;
        }
        for (int k = 0; k < grid.num_coupling_species_smc; k++) {
            smc.homo_flux(i, j, k) = 0.1;
            smc.hetero_flux(i, j, k) = 0.1;
        }
    }
}

/// Initial values found by running a sufficiently long simulation and recording state values
/// after they have reached a steady state.
void initialize_koenigsberger_ec(const grid_parms& grid, double* y, EC_type&  __restrict__ ec)
{
	int k, offset = (grid.neq_smc * grid.num_smc_circumferentially * grid.num_smc_axially);
	srand(grid.universal_rank);

	for (int i = 1; i <= grid.num_ec_circumferentially; i++) {
		for (int j = 1; j <= grid.num_ec_axially; j++) {
			k = offset + (i - 1) * grid.neq_ec_axially;

			if (grid.random)
			{
				y[k + ((j - 1) * grid.neq_ec) + ec_Ca] = (double)rand() / (double)(RAND_MAX / 0.3) + 0.7;
				y[k + ((j - 1) * grid.neq_ec) + ec_SR] = (double)rand() / (double)(RAND_MAX / 0.2) + 0.5;
				y[k + ((j - 1) * grid.neq_ec) + ec_Vm] = (double)rand() / (double)(RAND_MAX / 5.0) - 68.0;
				y[k + ((j - 1) * grid.neq_ec) + ec_IP3] = (double)rand() / (double)(RAND_MAX / 0.1) + 0.9;
				y[k + ((j - 1) * grid.neq_ec) + ec_Gprot] = (double)rand() / (double)(RAND_MAX / 50.0) + 1450;
			}
			else
			{
				y[k + ((j - 1) * grid.neq_ec) + ec_Ca] = 0.825;
				y[k + ((j - 1) * grid.neq_ec) + ec_SR] = 0.63;
				y[k + ((j - 1) * grid.neq_ec) + ec_Vm] = -66.7;
				y[k + ((j - 1) * grid.neq_ec) + ec_IP3] = 1.057;
				y[k + ((j - 1) * grid.neq_ec) + ec_Gprot] = 1470.305;
			}

		}
	}
	for (int i = 0; i < (grid.num_ec_circumferentially + grid.num_ghost_cells); i++) {
		for (int j = 0; j < (grid.num_ec_axially + grid.num_ghost_cells); j++) {
			ec.var(i, j, ec_Ca) = 0.0;
			ec.var(i, j, ec_SR) = 0.0;
			ec.var(i, j, ec_Vm) = 0.0;
			ec.var(i, j, ec_IP3) = 0.0;
			ec.var(i, j, ec_Gprot) = 0.0;

			for (int k = 0; k < grid.num_fluxes_ec; k++) {
				ec.flux(i, j, k) = 0.1;
			}
			for (int k = 0; k < grid.num_coupling_species_ec; k++) {
				ec.homo_flux(i, j, k) = 0.1;
				ec.hetero_flux(i, j, k) = 0.1;
			}
		}
	}
}

void koenigsberger_smc(const grid_parms& grid, SMC_type&  __restrict__ smc)
{
	koenigsberger_smc_implicit(grid, smc);
	koenigsberger_smc_explicit(grid, smc);
}

void koenigsberger_smc_derivatives(double* __restrict__ f, 
                                   const grid_parms& grid, SMC_type&  __restrict__ smc)
{
	koenigsberger_smc_derivatives_implicit(f, grid, smc);
	koenigsberger_smc_derivatives_explicit(f, grid, smc);
}

void koenigsberger_ec(const grid_parms& grid, EC_type&  __restrict__ ec)
{
	koenigsberger_ec_implicit(grid, ec);
	koenigsberger_ec_explicit(grid, ec);
}

void koenigsberger_ec_derivatives(double t, 
                                  double* __restrict__ f, const grid_parms& grid, 
                                  EC_type&  __restrict__ ec)
{
	koenigsberger_ec_derivatives_implicit(t, f, grid, ec);
	koenigsberger_ec_derivatives_explicit(t, f, grid, ec);
}

void koenigsberger_smc_implicit(const grid_parms& grid, SMC_type&  __restrict__ smc)
{
        const int na = grid.num_smc_axially;
        const int nc = grid.num_smc_circumferentially;
	// Evaluate single cell fluxes.
#pragma omp parallel for
	for (int ij = 0; ij < na * nc; ij++) {
        int i = ij / na + 1;
        int j = ij % na + 1;

        const double vSmc_Vm = smc.var(i, j, smc_Vm);
        const double vSmc_Ca = smc.var(i, j, smc_Ca);

		//Jvocc
		smc.flux(i, j, J_VOCC) = GCai * (vSmc_Vm - vCa1) / (1. + exp(-(vSmc_Vm - vCa2) / RCai));
		//J Na/Ca
		smc.flux(i, j, J_Na_Ca) = GNaCai * vSmc_Ca * (vSmc_Vm - vNaCai) / (vSmc_Ca + cNaCai);
		//JNa/K
		smc.flux(i, j, J_Na_K) = FNaK;
		//J Cl
		smc.flux(i, j, J_Cl) = GCli * (vSmc_Vm - vCl);
		//JK
		smc.flux(i, j, J_K) = GKi * smc.var(i, j, smc_w) * (vSmc_Vm - vKi);
	}
}

void koenigsberger_smc_explicit(const grid_parms& grid, SMC_type&  __restrict__ smc)
{
        const int na = grid.num_smc_axially;
        const int nc = grid.num_smc_circumferentially;
	// Evaluate single cell fluxes.
#pragma ivdep
#pragma omp parallel for
	for (int ij = 0; ij < na * nc; ij++) {
        int i = ij / na + 1;
        int j = ij % na + 1;

		const double vSmc_IP3 = smc.var(i, j, smc_IP3);
		const double vSmc_Ca = smc.var(i, j, smc_Ca);
		const double vSmc_SR = smc.var(i, j, smc_SR);
		const double vSmc_Vm = smc.var(i, j, smc_Vm);

		//JIP3
		smc.flux(i, j, J_IP3) = (Fi * P2(vSmc_IP3)) / (P2(Kri) + P2(vSmc_IP3));
		//JSRuptake
		smc.flux(i, j, J_SERCA) = (Bi * P2(vSmc_Ca)) / (P2(
					vSmc_Ca) + P2(cbi));
		//Jcicr
		smc.flux(i, j, J_CICR) = (CICRi * (P2(vSmc_SR) * P4(
					vSmc_Ca))) / ((P2(sci) + P2(vSmc_SR))
			* (P4(cci)+ P4(
							vSmc_Ca)));
		//Jextrusion
		smc.flux(i, j, J_Extrusion) = Di * vSmc_Ca * (1. + ((vSmc_Vm - vdi) / Rdi));

		//Jleak
		smc.flux(i, j, J_Leak) = Li * vSmc_SR;
		//Jvocc
		smc.flux(i, j, J_VOCC) = GCai * (vSmc_Vm - vCa1) / (1. + (exp(-(vSmc_Vm - vCa2) / RCai)));
		//J Na/Ca
		smc.flux(i, j, J_Na_Ca) = GNaCai * vSmc_Ca * (vSmc_Vm - vNaCai) / (vSmc_Ca + cNaCai);
		//Kactivation
		smc.flux(i, j, K_activation) = P2(vSmc_Ca + cwi) / (P2(
					vSmc_Ca + cwi) + beta * exp(-(vSmc_Vm - vCa3) / RKi));
		//Jdegradation
		smc.flux(i, j, J_IP3_deg) = ki * vSmc_IP3;
	}
}

void koenigsberger_smc_derivatives_implicit(double* __restrict__ f, 
                                            const grid_parms& grid, SMC_type&  __restrict__ smc)
{
#pragma ivdep
        for(int ij = 0; ij < grid.num_smc_circumferentially * grid.num_smc_axially; ij++) {
	        int i = ij / grid.num_smc_axially + 1;
            int j = ij % grid.num_smc_axially + 1;
            int k = (i - 1) * grid.neq_smc_axially;
            int ktot = k + (j - 1) * grid.neq_smc;

		    f[ktot + smc_Vm] = gama
					* (-smc.flux(i, j, J_Na_K) - smc.flux(i, j, J_Cl) - (2 * smc.flux(i, j, J_VOCC)) - smc.flux(i, j, J_Na_Ca) - smc.flux(i, j, J_K))
					+ smc.homo_flux(i, j, cpl_Vm) + smc.hetero_flux(i, j, cpl_Vm);

#if PLOTTING && EXPLICIT_ONLY
			if (i == SMC_COL && j == SMC_ROW && grid.universal_rank == RANK)
			{
				plotttingBuffer[bufferPos++] = smc.var(i, j, smc_Vm);
			}
#endif

#if ! EXPLICIT_ONLY
			f[ktot + smc_Ca] = 0.0;
			f[ktot + smc_SR] = 0.0;
			f[ktot + smc_w] = 0.0;
			f[ktot + smc_IP3] = 0.0;
#endif
	}
}

void koenigsberger_smc_derivatives_explicit(double* __restrict__ f, 
                                            const grid_parms& grid, SMC_type&  __restrict__ smc)
{
	for (int i = 1; i <= grid.num_smc_circumferentially; i++) {
		for (int j = 1; j <= grid.num_smc_axially; j++) {
			int ktot = (i - 1) * grid.neq_smc_axially + (j - 1) * grid.neq_smc;
			f[ktot + smc_Ca] = smc.flux(i, j, J_IP3) - smc.flux(i, j, J_SERCA) + smc.flux(i, j, J_CICR) - smc.flux(i, j, J_Extrusion)
					+ smc.flux(i, j, J_Leak) - smc.flux(i, j, J_VOCC) + smc.flux(i, j, J_Na_Ca) + smc.homo_flux(i, j, cpl_Ca) + smc.hetero_flux(i, j, cpl_Ca);

			f[ktot + smc_SR] = smc.flux(i, j, J_SERCA) - smc.flux(i, j, J_CICR) - smc.flux(i, j, J_Leak);

			f[ktot + smc_w] = lambda * (smc.flux(i, j, K_activation) - smc.var(i, j, smc_w));

			f[ktot + smc_IP3] = -smc.flux(i, j, J_IP3_deg) + smc.homo_flux(i, j, cpl_IP3) + smc.hetero_flux(i, j, cpl_IP3);

#if PLOTTING && EXPLICIT_ONLY
			if (i == SMC_COL && j == SMC_ROW && grid.universal_rank == RANK)
			{
				plotttingBuffer[bufferPos++] = smc.var(i, j, smc_Ca);
				plotttingBuffer[bufferPos++] = smc.var(i, j, smc_SR);
				plotttingBuffer[bufferPos++] = smc.var(i, j, smc_w);
				plotttingBuffer[bufferPos++] = smc.var(i, j, smc_IP3);
			}
#endif

#if ! EXPLICIT_ONLY
			f[ktot + smc_Vm] = 0.0;
#endif

		}
	}
}

void koenigsberger_ec_implicit(const grid_parms& grid, EC_type&  __restrict__ ec)
{
	// Evaluate single cell fluxes.
	for (int i = 1; i <= grid.num_ec_circumferentially; i++) {
		for (int j = 1; j <= grid.num_ec_axially; j++) {
			//Total K flux
			ec.flux(i, j, J_Ktot) = Gtot * (ec.var(i, j, ec_Vm) - vKj) * (ec.flux(i, j, J_BK_Ca) + ec.flux(i, j, J_SK_Ca));
			//Residual currents
			ec.flux(i, j, J_Residual) = GRj * (ec.var(i, j, ec_Vm) - vrestj);
		}
	}
}

void koenigsberger_ec_explicit(const grid_parms& grid, EC_type&  __restrict__ ec)
{
        const int na = grid.num_ec_axially;
        const int nc = grid.num_ec_circumferentially;
	// Evaluate single cell fluxes.
#pragma omp parallel for
	for (int ij = 0; ij < na * nc; ij++) {
        int i = ij / na + 1;
        int j = ij % na + 1;

		const double vEc_Ca = ec.var(i, j, ec_Ca);
		const double vEc_Vm = ec.var(i, j, ec_Vm);
		const double vEc_SR = ec.var(i, j, ec_SR);
		const double vEc_IP3 = ec.var(i, j, ec_IP3);
		const double log10_ec_Ca = log10(vEc_Ca);

		//JIP3
		ec.flux(i, j, J_IP3) = (Fj * P2(vEc_IP3)) / (P2(Krj) + P2(vEc_IP3));
		//JSRuptake
		ec.flux(i, j, J_SERCA) = (Bj * P2(vEc_Ca)) / (P2(vEc_Ca) + P2(cbj));
		//Jcicr
		ec.flux(i, j, J_CICR) = (CICRj * P2(vEc_SR) * P4(vEc_Ca))
		  / ((P2(vEc_SR) + P2(scj))*(P4(vEc_Ca)+P4(ccj)));
		//Jextrusion
		ec.flux(i, j, J_Extrusion) = Dj * vEc_Ca;
		//Jleak
		ec.flux(i, j, J_Leak) = Lj * vEc_SR;

		//IP3 degradation
		ec.flux(i, j, J_IP3_deg) = kDeg * vEc_IP3;

		//J_NonSelective Cation channels
		ec.flux(i, j, J_NSC) = (Gcatj * (ECa - vEc_Vm) * 0.5)
		  * (1 + ((double) (tanh((double) ((log10_ec_Ca - m3cat) / m4cat)))));
		//BK_channels
		ec.flux(i, j, J_BK_Ca) = (0.4 / 2.) * (1. + tanh(
				       ((log10_ec_Ca - c1j) * (vEc_Vm - bj) - a1j)
						 / ((m3b * (P2(vEc_Vm+(a2j*(log10_ec_Ca - c1j)) - bj))) + m4b)));
		//SK_channels
		ec.flux(i, j, J_SK_Ca) = (0.6 / 2.) * (1. + tanh((log10_ec_Ca - m3s) / m4s));
		//Grouping all other trivial Ca fluxes
		ec.flux(i, j, J_trivial_Ca) = J0j;

		// Ratio of bound to total P2Y
		ec.flux(i, j, L_P_P2Y) = ec.JPLC(i, j) / (ec.JPLC(i, j) + kATP); // TODO: Does this need to be calculated every time? is JPLC constant?

		// Rate of PIP2 hydrolysis.
		ec.flux(i, j, R_PIP2_H) = alpha_j * (vEc_Ca / (vEc_Ca + KCa)) * ec.var(i, j, ec_Gprot);

		// Induced IP3 influx
		ec.flux(i, j, J_ind_I) = (ec.flux(i, j, R_PIP2_H) * cons_PIP2 * unitcon_a) / (N_a * V_ec);

	}
}

void koenigsberger_ec_derivatives_implicit(double t, double* __restrict__ f, 
                                           const grid_parms& grid, EC_type&  __restrict__ ec)
{
	int offset = (grid.neq_smc * grid.num_smc_circumferentially * grid.num_smc_axially);
        for (int ij = 0; ij < grid.num_ec_circumferentially *  grid.num_ec_axially; ij++) 
	{
	        int i = ij / grid.num_ec_axially + 1;
                int j = ij % grid.num_ec_axially + 1;
                int ktot = offset + (i - 1) * grid.neq_ec_axially + (j - 1) * grid.neq_ec;

		f[ktot + ec_Vm] =
		      ((-1. / Cmj) * (ec.flux(i, j, J_Ktot) + ec.flux(i, j, J_Residual))) + ec.homo_flux(i, j, cpl_Vm) + ec.hetero_flux(i, j, cpl_Vm);
#if PLOTTING && EXPLICIT_ONLY
		if (i == EC_COL && j == EC_ROW && grid.universal_rank == RANK)
		{
			plotttingBuffer[bufferPos++] = ec.var(i, j, ec_Vm);
	        }
#endif

#if !EXPLICIT_ONLY

	       f[ktot + ec_Ca] = 0.0;
	       f[ktot + ec_SR] = 0.0;
	       f[ktot + ec_IP3] = 0.0;
	       f[ktot + ec_Gprot] = 0.0;
#endif
	}
}

void koenigsberger_ec_derivatives_explicit(double t, double* __restrict__ f, const grid_parms& grid, EC_type&  ec)
{
	int k, offset = (grid.neq_smc * grid.num_smc_circumferentially * grid.num_smc_axially);
	for(int i = 1; i <= grid.num_ec_circumferentially; i++)
	{
		for(int j = 1; j <= grid.num_ec_axially; j++)
		{
			k = offset + (i - 1) * grid.neq_ec_axially;

			f[k + ((j - 1) * grid.neq_ec) + ec_Ca] =
					ec.flux(i, j, J_IP3) - ec.flux(i, j, J_SERCA) + ec.flux(i, j, J_CICR) - ec.flux(i, j, J_Extrusion)
					+ ec.flux(i, j, J_Leak) + ec.flux(i, j, J_NSC) + ec.flux(i, j, J_trivial_Ca) + ec.homo_flux(i, j, cpl_Ca) + ec.hetero_flux(i, j, cpl_Ca);

			f[k + ((j - 1) * grid.neq_ec) + ec_SR] =
					ec.flux(i, j, J_SERCA) - ec.flux(i, j, J_CICR) - ec.flux(i, j, J_Leak);

			f[k + ((j - 1) * grid.neq_ec) + ec_IP3] =
						ec.flux(i, j, J_ind_I) - ec.flux(i, j, J_IP3_deg) + ec.homo_flux(i, j, cpl_IP3) + ec.hetero_flux(i, j, cpl_IP3);


			f[k + ((j - 1) * grid.neq_ec) + ec_Gprot] =
						(K_G_prot_act * (delta + ec.flux(i, j, L_P_P2Y)) * (G_prot_tot - ec.var(i, j, ec_Gprot)))
						- (K_G_prot_deact * ec.var(i, j, ec_Gprot));

#if PLOTTING && EXPLICIT_ONLY
			if (i == EC_COL && j == EC_ROW && grid.universal_rank == RANK)
			{
				plotttingBuffer[bufferPos++] = ec.var(i, j, ec_Ca);
				plotttingBuffer[bufferPos++] = ec.var(i, j, ec_SR);
				plotttingBuffer[bufferPos++] = ec.var(i, j, ec_IP3);

				plotttingBuffer[bufferPos++] = ec.var(i, j, ec_Gprot);
				plotttingBuffer[bufferPos++] = ec.flux(i, j, L_P_P2Y);
				plotttingBuffer[bufferPos++] = ec.flux(i, j, R_PIP2_H);
				plotttingBuffer[bufferPos++] = ec.flux(i, j, J_IP3_deg);
				plotttingBuffer[bufferPos++] = ec.flux(i, j, J_ind_I);
			}
#endif

#if ! EXPLICIT_ONLY
			f[k + ((j - 1) * grid.neq_ec) + ec_Vm] = 0.0;
#endif

		}
	}
}
